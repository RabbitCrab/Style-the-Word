...
parser.add_argument('--mode', type=int, default='1', help='1 for training, 0 for inference')
...
def plot_test_result(real_image, gen_image, recon_image, count, save=True,  show=False, fig_size=(15, 15)):
    ...
    if save:
        save_fn = './sakura_test/count{:d}'.format(count+1) + '.png'
        plt.savefig(save_fn)
...
def plot_train_result(real_image, gen_image, recon_image, epoch, save=True,  show=False, fig_size=(15, 15)):
    ...
    if save:
        save_fn = './sakura_fig/Result_epoch_{:d}'.format(epoch+1) + '.png'
        plt.savefig(save_fn)
...
if args.mode == 1:
    print('Train')
    train_data_A = DatasetFromFolder(data_dir, subfolder='train_normal', transform=transform,
    ...
    train_data_B = DatasetFromFolder(data_dir, subfolder='train_sakura', transform=transform,
                                    resize_scale=params['resize_scale'], crop_size=params['crop_size'], fliplr=params['fliplr'])
    train_data_loader_B = torch.utils.data.DataLoader(dataset=train_data_B, batch_size=params['batch_size'], shuffle=True)
    
    test_data_A = DatasetFromFolder(data_dir, subfolder='val_normal', transform=transform)
    test_data_loader_A = torch.utils.data.DataLoader(dataset=test_data_A, batch_size=params['batch_size'], shuffle=False)
    test_data_B = DatasetFromFolder(data_dir, subfolder='val_sakura', transform=transform)
    test_data_loader_B = torch.utils.data.DataLoader(dataset=test_data_B, batch_size=params['batch_size'], shuffle=False)



    
    test_real_A_data = train_data_A.__getitem__(12).unsqueeze(0)
    test_real_B_data = train_data_B.__getitem__(90).unsqueeze(0)
    
    G_A = Generator(3, params['ngf'], 3, params['num_resnet']).cuda()
    G_B = Generator(3, params['ngf'], 3, params['num_resnet']).cuda()

    D_A = Discriminator(3, params['ndf'], 1).cuda()
    ...
    all_losses.to_csv('sakura_avg_losses', index=False)


    torch.save(G_B.state_dict(), './weight/sakura/G_B.pth')
    torch.save(G_A.state_dict(), './weight/sakura/G_A.pth')
    torch.save(D_B.state_dict(), './weight/sakura/D_B.pth')
    torch.save(D_A.state_dict(), './weight/sakura/D_A.pth')
    torch.save(G_optimizer.state_dict(), './weight/sakura/G_opt.pth')
    torch.save(D_A_optimizer.state_dict(), './weight/sakura/D_A_opt.pth')
    torch.save(D_B_optimizer.state_dict(), './weight/sakura/D_B_opt.pth')
    ...
    G_A.load_state_dict(torch.load('./sakura/G_A.pth'))
    G_B.load_state_dict(torch.load('./sakura/G_B.pth'))


    test_data_A = DatasetFromFolder(data_dir, subfolder='test_normal', transform=transform)
    test_data_loader_A = torch.utils.data.DataLoader(dataset=test_data_A, batch_size=params['batch_size'], shuffle=False)
    test_data_B = DatasetFromFolder(data_dir, subfolder='test_sakura', transform=transform)
    test_data_loader_B = torch.utils.data.DataLoader(dataset=test_data_B, batch_size=params['batch_size'], shuffle=False)
    ...
    with torch.no_grad():
        ...
        for i, (real_A, real_B) in enumerate(zip(test_data_loader_A, test_data_loader_B)):
            ...
            test_fake_B = G_A(real_A)
            test_recon_A = G_B(test_fake_B)

            test_fake_A = G_B(real_B)
            test_recon_B = G_A(test_fake_A)

            count += 1
            
            plot_test_result([real_A, real_B], [test_fake_B, test_fake_A], [test_recon_A, test_recon_B],
                                count, save=True)
